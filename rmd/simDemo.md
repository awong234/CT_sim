...
The population above has a `psi.bar` value of 0.99. Recall the formula above for `psi.bar`: this is the expected occupancy probability over the entire area, over 10 occasions. Basically, if you went out to a random pixel and sampled it 10 times, you would expect to see an animal 0.99 times, assuming detection is perfect.
